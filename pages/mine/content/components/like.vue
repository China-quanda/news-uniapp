  ...
  <view class="my-like">
    <view v-if="list.length" class="item-avatar-one" v-for="item in list" :key="item.id">
        <view class="top"  @click="goToUser(item.article.user.id)">
         <u-avatar :src="item.article.user.avatar ? item.article.user.avatar : avatar"  size="36"></u-avatar>
      ...
      <my-share></my-share>
			...
			<view v-if="!list.length" class="empty">
				<u-empty mode="list"  text="暂无点赞内容"/>
			</view>
    </view>
</template>

<script>
import {getUserArticleLikings} from '@/api/article'
export default {
  name: 'my-like',
  ...
  created () {
    this.getUserArticleLikings(this.userId,this.query)
  },
  methods: {
    goToUser (id) {
    ...
    async getUserArticleLikings(id,data){
       const res = await getUserArticleLikings(id,data)
