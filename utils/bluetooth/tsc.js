	...
	createNew: function() {
		...
		jpPrinter.setBitmap = function(x, y, mode, res) { //添加图片，res为画布参数
			...
			for (var i = 0; i < height; ++i) {
				...
				for (var j = 0; j < width; ++j) {
					for (var k = 0; k < 32; k += 4) {
						if (res.data[temp] == 0 && res.data[temp + 1] == 0 && res.data[temp + 2] == 0 && res.data[temp + 3] ==
							0) {
							pointList.push(1)
						} else {
							pointList.push(0)
			...
			for (var i = 0; i < pointList.length; i += 8) {
				var p = pointList[i] * 128 + pointList[i + 1] * 64 + pointList[i + 2] * 32 + pointList[i + 3] * 16 +
					pointList[i + 4] * 8 + pointList[i + 5] * 4 + pointList[i + 6] * 2 + pointList[i + 7]
