...
(function(vue) {
      ...
      return function(t) {
          ...
          class d extends i {
            ...
            doOpen() {
              ...
              try {
                ...
                return formatAppLog("log", "at node_modules/.pnpm/registry.npmmirror.com+@hyoga+uni-socket.io@3.0.4/node_modules/@hyoga/uni-socket.io/dist/uni-socket.io.js:1", "error", t3), this.emit("error", t3);
              }
        ...
        function p(t2, e2, n2, r2) {
          ...
          if (h(n2), void 0 === (s2 = t2._events) ? (s2 = t2._events = /* @__PURE__ */ Object.create(null), t2._eventsCount = 0) : (void 0 !== s2.newListener && (t2.emit("newListener", e2, n2.listener ? n2.listener : n2), s2 = t2._events), i2 = s2[e2]), void 0 === i2)
            ...
            c2.name = "MaxListenersExceededWarning", c2.emitter = t2, c2.type = e2, c2.count = i2.length, a2 = c2, console && console.warn && formatAppLog("warn", "at node_modules/.pnpm/registry.npmmirror.com+@hyoga+uni-socket.io@3.0.4/node_modules/@hyoga/uni-socket.io/dist/uni-socket.io.js:10", a2);
          }
          return t2;
        }
        function f() {
  ...
  const storage = {
    ...
    get: (name) => {
      try {
        const data2 = uni.getStorageSync(name);
  ...
  const request = (config) => {
    ...
    formatAppLog("log", "at utils/request.ts:10", config);
    return new Promise((resolve, reject) => {
      uni.request({
        url: config.baseUrl || request$1.baseUrl + config.url,
        method: config.method || "get",
        timeout: config.timeout || request$1.timeout,
        data: config.data,
        header: {
          // 'Authorization': storage.get('token') || null,
          ...config.header
        },
        dataType: request$1.dataType,
        success: (res) => {
          if (config.data && config.data.Loading) {
            uni.hideLoading();
          }
        ...
        complete: (res) => {
          formatAppLog("log", "at utils/request.ts:83", "complete", res);
          formatAppLog("log", "at utils/request.ts:84", "config", config);
  ...
  const _sfc_main$3 = /* @__PURE__ */ vue.defineComponent({
    __name: "test",
    setup(__props) {
      storage.set("token", "877880098@qq.com");
      const scanCode = () => {
        uni.scanCode({
          onlyFromCamera: true,
          success: (res) => {
            ...
            if (result.type == "login") {
              qrScannedLogin({ qrcodeId: result.qrcodeId }).then((res2) => {
                formatAppLog("log", "at pages/test/test.vue:27", res2);
                storage.set("qrCode", { ...result, ...res2 });
                router.push({ url: "/pages/login/confirmLogin" });
              }).catch((err) => {
                formatAppLog("log", "at pages/test/test.vue:31", err);
              });
            }
          }
        });
      ...
      const requestApi = () => {
        ...
        uni.request({
          url: url1,
          success: (res) => {
            formatAppLog("log", "at pages/test/test.vue:44", "连接api接口成功", res.data);
          },
          fail: (e) => {
            formatAppLog("log", "at pages/test/test.vue:47", "连接api接口失败");
          }
        });
      };
      const requestFn = () => {
        qrScannedLogin({ qrcodeId: 42 }).finally((res) => {
          formatAppLog("log", "at pages/test/test.vue:53", "res", res);
        }).then((rr) => {
          formatAppLog("log", "at pages/test/test.vue:55", rr);
        });
      };
      onLoad((option) => {
        requestApi();
        ...
        socket.on("connect", () => {
          formatAppLog("log", "at pages/test/test.vue:111", "ws 已连接");
        });
        socket.emit("words", { time: "lisi" });
        socket.on("words", (msg) => {
          formatAppLog("log", "at pages/test/test.vue:115", "words words", msg);
        });
        socket.on("error", (msg) => {
          formatAppLog("log", "at pages/test/test.vue:118", "ws error", msg);
        });
      });
      onHide(() => {
      });
      return (_ctx, _cache) => {
        return vue.openBlock(), vue.createElementBlock("view", null, [
          vue.createElementVNode("button", { onClick: scanCode }, "扫码"),
          vue.createElementVNode("button", { onClick: requestApi }, "请求"),
          vue.createElementVNode("button", { onClick: requestFn }, "请求request接口测试")
        ]);
      };
    }
  });
  ...
  const PagesTestTest = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__file", "/Users/a111/Desktop/xm/news-app/pages/test/test.vue"]]);
  const _sfc_main$2 = {
    data() {
      return {
        title: "Hello"
  ...
  const PagesIndexIndex = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["render", _sfc_render], ["__file", "/Users/a111/Desktop/xm/news-app/pages/index/index.vue"]]);
    ...
    setup(__props) {
      ...
      let login = () => {
        ...
        qrConfirmLogin({ qrcodeId: qrCode.qrcodeId }).then((res) => {
          formatAppLog("log", "at pages/login/confirmLogin.vue:25", res);
          router.back();
        }).catch((err) => {
          formatAppLog("log", "at pages/login/confirmLogin.vue:28", err);
        }).finally(() => {
          loading.value = false;
        });
      };
      let cancel = () => {
        qrCancelLogin({ qrcodeId: qrCode.qrcodeId }).then((res) => {
          formatAppLog("log", "at pages/login/confirmLogin.vue:35", res);
        }).catch((err) => {
          formatAppLog("log", "at pages/login/confirmLogin.vue:37", err);
        }).finally(() => {
          router.back();
        });
      };
      ...
      return (_ctx, _cache) => {
        return vue.openBlock(), vue.createElementBlock("view", { class: "container" }, [
          ...
          vue.createElementVNode(
            "view",
            { class: "tips" },
            " 当前账号正在尝试登录" + vue.toDisplayString(vue.unref(app).title) + "网页端，请确认信息及登录行为",
            1
            /* TEXT */
          ),
          vue.createElementVNode("button", {
            ...
            (...args) => vue.unref(login) && vue.unref(login)(...args))
          }, vue.toDisplayString(vue.unref(loginText)), 9, ["loading"]),
          vue.createElementVNode("button", {
            type: "default",
  ...
  const PagesLoginConfirmLogin = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-7aef36b4"], ["__file", "/Users/a111/Desktop/xm/news-app/pages/login/confirmLogin.vue"]]);
  ...
  const App = /* @__PURE__ */ _export_sfc(_sfc_main, [["__file", "/Users/a111/Desktop/xm/news-app/App.vue"]]);
  function createApp() {
